      ...
      <main className="flex-1 flex items-center justify-center p-4 sm:p-6 md:p-8">
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-2xl p-6 sm:p-8 md:p-10 max-w-lg w-full text-center">
          ...
          <div className="space-y-4 sm:space-y-5">
            {/* Create Room Button - Host colors (purple/pink) */}
            <motion.button
              onClick={onCreateRoom}
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
              transition={{ type: 'spring', stiffness: 400, damping: 17 }}
              className="w-full bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white font-bold py-4 sm:py-5 px-6 rounded-xl shadow-lg hover:shadow-xl transition-all text-lg sm:text-xl"
            >
